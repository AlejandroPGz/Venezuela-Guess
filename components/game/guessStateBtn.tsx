...
export default function GuessStateBtn({closeModal}) {
  ...
  return (
    ...
    <Text className="text-xl text-center font-bold text-cloudy-50">Adivina el estado según su capital</Text>
    <TouchableOpacity
    onPress={() => {
      closeModal();
      router.push("guessState/1");
    }}
      className="bg-cloudy-100 w-4/5 py-2 px-6 rounded-2xl justify-center items-center mt-4"
      >
        <Text className="text-lg font-bold w-full text-center">Nivel 1</Text>
        {/* <Text className="text-lg font-bold w-full text-center">Mejor Tiempo: 44s</Text> */}
      </TouchableOpacity>
      <TouchableOpacity
         onPress={() => {
          closeModal();
          router.push("guessState/2");
        }}
